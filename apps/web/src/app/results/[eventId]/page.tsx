...
export default function EventResults({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  return <main className="race-page"><div className="race-topline" /><RaceNav links={[{ href: "/events", label: "All events" }]} /><header className="race-masthead"><div className="mx-auto max-w-2xl"><p className="race-kicker">SplitSync // event results</p><h1 className="race-title">{event.title}</h1><p className="mt-1 text-xs font-bold uppercase tracking-wide text-race-muted">{event.location}</p></div></header><div className="mx-auto max-w-2xl px-4 py-8 sm:px-6">
    ...
    <div className="race-section-heading"><p className="race-kicker">Live classifications</p><h2 className="mt-1 text-2xl font-black uppercase">Race results</h2></div>{races.length === 0 ? <p className="py-10 text-center text-sm font-bold uppercase tracking-wide text-race-muted">Race schedule coming soon</p> : <ul className="mt-5 border-t-2 border-race-ink">{races.map((race) => <li key={race.id} className="flex items-center justify-between gap-3 border-b border-zinc-300 bg-race-panel px-4 py-4 even:bg-race-panel-alt"><div className="min-w-0"><p className="truncate text-base font-black uppercase">{race.name}</p><p className="mt-1 text-xs font-bold text-race-muted">{race.is_time_trial ? "Time trial" : race.laps_planned ? `${race.laps_planned} laps` : "Timed race"}</p></div><div className="flex shrink-0 items-center gap-3"><span className={`px-2 py-1 text-[10px] font-black uppercase tracking-wide ${statusStyle[race.status]}`}>{race.status === "active" ? "Live" : race.status}</span><Link href={`/live/${race.id}`} className="race-action">View</Link></div></li>)}</ul>}<p className="mt-6 text-center text-[10px] font-bold uppercase tracking-[0.14em] text-race-muted">Live unofficial timing · SplitSync</p></div></main>;
}
