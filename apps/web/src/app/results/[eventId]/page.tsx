...
function RaceListItem({ race }: { race: Race }) {
  ...
  return (
    ...
    >
      <div className="min-w-0">
        <p className="truncate text-base font-black uppercase">{race.name}</p>
        <p className="mt-1 text-xs font-bold text-race-muted">
          {race.is_time_trial
            ? "Time trial"
            : race.laps_planned
              ? <><span className="race-numeral">{race.laps_planned}</span> laps</>
              : "Timed race"}
        </p>
      </div>
      <div className="flex shrink-0 items-center gap-3">
        ...
        >
          {isActive && <span className="race-live-pulse" aria-hidden="true" />}
          {isActive ? "Live" : race.status}
        </span>
        <Link href={`/live/${race.id}`} className="race-action">View</Link>
      </div>
    </li>
  );
}
...
export default function EventResults({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  return (
    <main className="race-page">
      ...
      <RaceNav links={[{ href: "/events", label: "All events" }]} />
      <header className="race-masthead">
        <div className="mx-auto max-w-2xl">
          <p className="race-kicker">SplitSync // event results</p>
          <h1 className="race-title">{event.title}</h1>
          <p className="mt-1 text-xs font-bold uppercase tracking-wide text-race-muted">{event.location}</p>
        </div>
      </header>
      <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6">
        {hasDetails && (
          <div className="race-panel mb-6 overflow-hidden">
        ...
        <div className="race-section-heading">
          <p className="race-kicker">Live classifications</p>
          <h2 className="mt-1 text-2xl font-black uppercase">Race results</h2>
        </div>
        {races.length === 0 ? (
          <p className="py-10 text-center text-sm font-bold uppercase tracking-wide text-race-muted">Race schedule coming soon</p>
        ) : (
          <ul className="mt-5 border-t-2 border-race-ink">
            {races.map((race) => <RaceListItem key={race.id} race={race} />)}
          </ul>
        )}
        <p className="mt-6 text-center text-[10px] font-bold uppercase tracking-[0.14em] text-race-muted">Live unofficial timing · SplitSync</p>
      </div>
    </main>
  );
}
